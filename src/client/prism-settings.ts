...
export const MATERIALS: Readonly<Record<PrismMaterial, MaterialRecipe>> = Object.freeze({
...
export const FONT_CHOICES: Readonly<Record<PrismFontChoice, string>> = Object.freeze({
  system: '',
...
export const CODE_FONT_CHOICES: Readonly<Record<PrismFontChoice, string>> = Object.freeze({
  system: '',
...
export const DEFAULT_ZONES: PrismZones = Object.freeze({
  base: 100,
  sidebar: 100,
  card: 100,
  input: 100,
...
export const SETTINGS_DEFAULTS: PrismSettings = Object.freeze({
  enabled: true,
  material: 'frosted',
  blur: 18,
  frost: 50,
...
export const PRESETS: Readonly<Record<PrismPreset, PresetRecipe>> = Object.freeze({
...
function wallpaperList(value: unknown): string[] {
  if (typeof value === 'string') return value === '' ? [] : [value]
  if (Array.isArray(value)) return value.filter((item): item is string => typeof item === 'string' && !item.startsWith('idb:'))
  return []
}

/** Normalize one luminance slot (mirrors the wallpaper list length). */
function dimList(value: unknown, fallbackCount: number): number[] {
  if (typeof value === 'number') return [value]
  const list = Array.isArray(value) ? value.filter((item): item is number => typeof item === 'number') : []
  const out = list.slice(0, fallbackCount)
  while (out.length < fallbackCount) out.push(0.5)
  return out
}
...
function zonesField(value: unknown): PrismZones {
  const stored = typeof value === 'object' && value !== null ? value as Partial<PrismZones> : {}
  return {
 ...
 */
export function readSettings(): PrismSettings {
  try {
    const raw = localStorage.getItem(PRISM_SETTINGS_KEY)
    if (raw === null) return { ...SETTINGS_DEFAULTS, zones: { ...DEFAULT_ZONES } }
    const parsed = JSON.parse(raw) as Record<string, unknown> & Partial<PrismSettings>
...
export function writeSettings(settings: PrismSettings): void {
  try {
 ...
 */
export function isPrismStorageKey(key: string | null): boolean {
