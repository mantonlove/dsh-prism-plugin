...
function clampChannel(value: number, min: number, max: number, fallback: number): number {
 ...
 * @returns hex string.
 */
export function hslToHex(color: ColorHSL): string {
...
export function hexToHsl(hex: string): ColorHSL {
  ...
  if (!/^[0-9a-fA-F]{6}$/.test(value)) return { h: 0, s: 0, l: 50 }
